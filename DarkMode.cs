...
namespace DarkModeForms
{
    ...
    public class DarkModeCS
    {
        ...
        {
            foreach (Form form in MainEditor.GetAllForms())
            {
                try
                {
                    ...
                    OScolors = GetSystemColors(form, pIsDarkMode ? 0 : 1);

                    if (OScolors != null)
                    {
                        //Apply Window's Dark Mode to the Form's Title bar:
                        ApplySystemDarkTheme(form, pIsDarkMode);

                        form.BackColor = OScolors.Background;
                        form.ForeColor = OScolors.TextInactive;
                        ...
                        if (form != null && form.Controls != null)
                        {
                            ...
                            form.ControlAdded -= ownerFormControlAdded; //prevent uncontrolled multiple addition
                            form.ControlAdded += ownerFormControlAdded;
                        }
        ...
        public void ApplyTheme(Form form, bool pIsDarkMode = true)
        {
            try
            {
                // IsDarkMode member changes only if the User manually changed it here:
